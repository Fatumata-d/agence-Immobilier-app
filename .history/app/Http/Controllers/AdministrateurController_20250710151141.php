...
class AdministrateurController extends Controller
{
    ...
    public function rapport() 
    {
        ...
        $clientsAnnee = Client::whereYear('created_at', Carbon::now()->year)->count();
         ...
         $biensVendus = BienImmobilier::where('statut', 'vendu')->get(); 
         $agentsInactifs = Agent::where('statut', 0)->get();

        $biensParStatut = BienImmobilier::selectRaw('statut, COUNT(*) as total')
    ->groupBy('statut')
    ->pluck('total', 'statut'); // ça retourne un tableau ['disponible' => 5, 'vendu' => 3, ...]


        return view('Rapport', compact(
            'reservationsJour', 'reservationsMois', 'reservationsAnnee',
            'clientsJour', 'clientsMois', 'clientsAnnee', 'biensVendus', 'agentsInactifs'
        ));
    }
}
